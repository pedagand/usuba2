...
let to_slice spec =
  ...
  in
  match slices with [ s0; s1; s2; s3 ] -> (s0, s1, s2, s3) | _ -> assert false
...
let slice_of_bools bools =
  ...
  Ua0.Value.VBool (List.nth bools index)
...
let pp format =
  List.iter (fun cipher ->
      let () =
        List.iter (fun c -> Format.fprintf format "%02x " @@ Char.code c) cipher
      in
