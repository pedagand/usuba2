  ...
  run "typesynth"
    [
      ...
      ( "Var",
        [
          test_case "in-Bool" `Quick (fun () ->
              check_typesynth Env0 Term.S.(v x) Ty.S.bool);
          test_case "in-Var" `Quick (fun () ->
              check_typesynth Env0 Term.S.(v y) Ty.S.(v alpha));
          test_case "in-App" `Quick (fun () ->
              check_typesynth Env0 Term.S.(v z) Ty.S.(apps [ _F; _G ] bool));
        ] );
      ( "Fn",
        [
      ...
      ( "Lookup",
        [
          test_case "in range" `Quick (fun () ->
              check_typesynth Env0 Term.S.((v z).%(2)) Ty.S.(_G @ bool));
          test_case "out of range" `Quick (fun () ->
              fail_typesynth Env0 Term.S.((v z).%(4)));
          test_case "not Naperian" `Quick (fun () ->
              fail_typesynth Env0 Term.S.((v x).%(2)));
        ] );
      ( "Operator",
        [
          test_case "xor" `Quick (fun () ->
              check_typesynth Env0 Term.S.(s (v x) lxor s (v x)) Ty.S.bool);
          test_case "no ad-hoc polymorphism" `Quick (fun () ->
              fail_typesynth Env0 Term.S.(s (v x) lxor s (v y)));
        ] );
      ( "FnCall",
        [
      ...
      ( "Ann",
        [
          test_case "well-typed" `Quick (fun () ->
              check_typesynth Env0 Term.S.(ann true' Ty.S.bool) Ty.S.bool);
          test_case "ill-typed" `Quick (fun () ->
              fail_typesynth Env0 Term.S.(ann (s (vfn f)) Ty.S.bool));
        ] );
      ( "Circ",
        [
          test_case "well-typed" `Quick (fun () ->
      ...
      ( "Reindex",
        [
          ...
          test_case "not applicative" `Quick (fun () ->
              fail_typesynth Env0 Term.S.(reindex [ _F ] [ _G ] (v x)));
          test_case "lhs mismatch" `Quick (fun () ->
              fail_typesynth Env0 Term.S.(reindex [ _G ] [ _G ] (v z)));
          test_case "rhs mismatch" `Quick (fun () ->
              fail_typesynth Env0 Term.S.(reindex [ _F ] [ _F ] (v z)));
        ] );
      ( "Lift",
        [
          test_case "well-typed" `Quick (fun () ->
      ...
      ( "True/False",
        [
          test_case "true well-typed" `Quick (fun () ->
              check_typecheck Env0 Term.S.true' Ty.S.bool);
          test_case "false well-typed" `Quick (fun () ->
              check_typecheck Env0 Term.S.false' Ty.S.bool);
          test_case "ill-typed" `Quick (fun () ->
              fail_typecheck Env0 Term.S.true' Ty.S.(v alpha));
        ] );
      ( "Constructor",
        [
          test_case "well-typed" `Quick (fun () ->
              check_typecheck Env0
                Term.S.(cstr _F [ true'; false'; true'; true' ])
                Ty.S.(_F @ bool));
          test_case "ill-typed" `Quick (fun () ->
              fail_typecheck Env0
                Term.S.(cstr _F [ true'; false'; true'; true' ])
          ...
          test_case "arity checked" `Quick (fun () ->
              fail_typecheck Env0
                Term.S.(cstr _F [ false'; true'; true' ])
                Ty.S.(_F @ bool));
        ] );
      ( "Let",
        [
          ...
          test_case "ill-typed application" `Quick (fun () ->
              fail_typecheck Env0
                Term.S.(let' "a" (v y) (fun a -> s (s a land s a)))
                Ty.S.bool);
          test_case "rejecting wrong type" `Quick (fun () ->
              fail_typecheck Env0
                Term.S.(let' "a" (v y) (fun _a -> s (v x)))
                Ty.S.(v alpha));
        ] );
      ( "Let+",
        [
